...
const ActiveFlowMeter = () => {
  ...
  useEffect(() => {
    const fetchColumns = async () => {
      try {
        const response = await axios.get(`${apiurl}v1/api/get-columns?division=active&parttype=flow-meter`);
  ...
  const fetchParts = async () => {
    try {
      const response = await axios.get(`${apiurl}v1/api/flow-meter-active/parts`);
      if (response.data.success) {
  ...
  const handleUpdate = async (sr_no, updatedFields) => {
    try {
      await axios.patch(`${apiurl}v1/api/flow-meter-active/parts/${sr_no}`, updatedFields);
  ...
  const handleDelete = async (sr_no) => {
    try {
      await axios.delete(`${apiurl}v1/api/flow-meter-active/parts/${sr_no}`);
      fetchParts(); // Re-fetch the parts to update the table
    } catch (error) {
...
export default ActiveFlowMeter;