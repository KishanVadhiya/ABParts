...
const SpareControlValve = () => {
  ...
  useEffect(() => {
    const fetchColumns = async () => {
      try {
        const response = await axios.get(`${apiurl}v1/api/get-columns?division=spare&parttype=control-valve`);
  ...
  const fetchParts = async () => {
    try {
      const response = await axios.get(`${apiurl}v1/api/control-valve-spare/parts`);
      if (response.data.success) {
  ...
  const handleUpdate = async (sr_no, updatedFields) => {
    try {
      await axios.patch(`${apiurl}v1/api/control-valve-spare/parts/${sr_no}`, updatedFields);
      fetchParts();
      setEditingPart(null);
    } catch (error) {
  ...
  const handleDelete = async (sr_no) => {
    try {
      await axios.delete(`${apiurl}v1/api/control-valve-spare/parts/${sr_no}`);
      fetchParts();
    } catch (error) {
  ...
  const handleTransferClick = async (part) => {
    ...
    try {
      const response = await axios.get(`${apiurl}v1/api/control-valve-active/departments`);
      if (response.data.success) {
  ...
  const handleTransferSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${apiurl}v1/api/control-valve-active/parts/from-spare/${transferringPart}`, { department: selectedDepartment });
      fetchParts();
...
export default SpareControlValve;